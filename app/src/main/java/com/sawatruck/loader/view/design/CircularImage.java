 ...
 */
public class CircularImage extends android.support.v7.widget.AppCompatImageView {
    ...
    public CircularImage(final Context context) {
        this(context, null);
    }
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.CircularView, defStyleAttr, 0);

        setBackGroundColor(attributes.getColor(R.styleable.CircularView_circular_color,backGroundColor));
        // Init Border
        if (attributes.getBoolean(R.styleable.CircularView_circular_border, true) ) {
            float defaultBorderSize = DEFAULT_BORDER_WIDTH * getContext().getResources().getDisplayMetrics().density;
            setBorderWidth(attributes.getDimension(R.styleable.CircularView_circular_border_width, defaultBorderSize));
            setBorderColor(attributes.getColor(R.styleable.CircularView_circular_border_color, DEFAULT_BORDER_COLOR));
        }

        // Init Shadow
        if (attributes.getBoolean(R.styleable.CircularView_circular_shadow, false)) {
            shadowRadius = DEFAULT_SHADOW_RADIUS;
            drawShadow(attributes.getFloat(R.styleable.CircularView_circular_shadow_radios, shadowRadius), attributes.getColor(R.styleable.CircularView_CircularView_circular_shadow_color, shadowColor));
        }
    }
    //endregion
    ...
    public void setBackGroundColor(int borderColor) {
        if (paint != null)
    ...
    private Bitmap cropBitmap(Bitmap bitmap) {
        Bitmap bmp;
        Logger.error(String.format("bitmap width = %d and height = %d", bitmap.getWidth(), bitmap.getHeight()));
